...
Loop de treino da GCN para o dataset BBBP.

Como rodar (da raiz do projeto):
    python src/training/train.py

O que este script faz:
    1. Carrega o dataset BBBP e aplica scaffold split estratificado
    2. Inicializa a GCN e o otimizador
    3. Treina por NUM_EPOCHS épocas, avaliando em val a cada época
    4. Salva o melhor modelo (por AUC de validação) em models/checkpoints/
    5. Salva as métricas de treino em results/metrics/gcn_training.json
"""
...
WEIGHT_DECAY    = 1e-4
NUM_EPOCHS      = 100
PATIENCE        = 15       # early stopping: para se val_auc não melhorar por N épocas
POS_WEIGHT      = 3.29     # 1567/479 ≈ proporção inversa BBB+/BBB- para BCEWithLogitsLoss

CHECKPOINT_PATH = "models/checkpoints/gcn_best.pt"
METRICS_PATH    = "results/metrics/gcn_training.json"
...
def main():
    ...
    # 4. Loop de treino
    print(f"\nTreinando GCN por até {NUM_EPOCHS} épocas (early stopping={PATIENCE})...\n")
    best_val_auc  = 0.0
    epochs_no_improve = 0
    history = []
    ...
    print(f"GCN (grafos): AUC = {test_auc:.4f}")
    print(f"{'='*45}")
